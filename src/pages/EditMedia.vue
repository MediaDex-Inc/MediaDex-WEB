...
import { getMediaById, getMediaTags, getMediaCustomFields, createMediaTagLink, updateMedia, removeMediaTagLink } from '@/ts/api/media'
...
const removeTag = (n: string) => {
    const tag = tags.value.find(t => t.name === n)
    if (tag?.tag_id) deletedTagIds.value.push(tag.tag_id)
    tags.value = tags.value.filter(t => t.name !== n)
}

/* ---------------- Custom Fields -----------------------*/
const deletedFieldIds = ref<number[]>([])
const customFields = ref<{ name: string; value: string; field_id?: number }[]>([])
...
const removeCustomField = (index: number) => {
    ...
    if (field?.field_id) deletedFieldIds.value.push(field.field_id)
...
onMounted(async () => {
    try {
        const [media, fetchedTags, fields] = await Promise.all([
            getMediaById(mediaId),
            getMediaTags(mediaId),
            getMediaCustomFields(mediaId),
        ])
        ...
        const fieldValues = await Promise.all(
            fields.map(async field => {
                const result = await getMediaFieldValue(field.field_id, mediaId)
                return { field_id: field.field_id, name: field.name, value: result?.value ?? '' }
            })
        )
...
const handleSubmit = async () => {
    if (!validate()) return
    try {
        for (const tagId of deletedTagIds.value) {
            await removeMediaTagLink(mediaId, tagId);
        }

        for (const fieldId of deletedFieldIds.value) {
            await deleteMediaField(fieldId, mediaId);
        ...
        await updateMedia(mediaId, mediaPayload)
...
<template>
    <div class="media-card">
        ...
        <div class="card-verso">
            ...
            <button class="btn-submit" @click="handleSubmit">Save changes</button>
        </div>
    </div>
</template>
